...
describe("Sui SDK tests", () => {
  ...
  test("Transfer Sui token with payload to Ethereum", async () => {
    ...
    // Create the EmitterCap
    const newEmitterCapTx = newEmitterCap(
      suiCoreBridgePackageId,
      SUI_CORE_BRIDGE_STATE_OBJECT_ID,
      suiAddress
    );
    result = await executeTransactionBlock(suiSigner, newEmitterCapTx);
    result.effects?.status.status === "failure" &&
      console.log(JSON.stringify(result.effects, null, 2));
    console.log(result);
    expect(result.effects?.status.status).toBe("success");
    ...
    expect(emitterCapObjectId).toBeTruthy();
